...
public class UpdateMemcachedTestNaturalTest extends MemcachedTest {
  private static final Logger logger = Logger.getLogger(UpdateMemcachedTestNaturalTest.class);
  ...
  public void testUpdate() throws IOException, SQLException {
    ...
    try {
      ...
      // Check status flag.
      assertTrue("Inserted DTO does not have status EXISTING: " + dto.getStatus(),
      ...
      UpdateMemcachedByKey<TestNatural> updater =
        new UpdateMemcachedByKey<>(new UpdateSqlTestNatural(), getSubsystem());
      ...
      if (!rs.next()) {
        ...
        assertTrue("Col 1 not updated from " + col1 + " to " + col1Updated + ": " + dbCol1,
      ...
      assertTrue("TestNatural object not removed from memcached cache", object == null);
